...
int forma_par(char f, char d){  // Caractere de fechamente e caracteres desempilhado
    ...
    switch(f){  // Switch para verificar se caracter de fehcamente é igual ao do topo da pilha se for retorna True se não for retornar False
        case ')':
            if (d  == '(' )
...
int identifica_formacao(char x[]){  // Recebe a string com a expressão
    ...
    while(x[i] != '\n'){  // Faz um loop até o fim do arraya
